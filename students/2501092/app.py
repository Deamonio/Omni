...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://s2501092:hanbyul0623@localhost/student_db'
...
class User(db.Model):
    ...
    hobby = db.Column(db.String(100))   # [추가됨] 취미 컬럼
...
def signup():
    if request.method == 'POST':
        ...
        new_user = User(
            ...
            hobby=request.form.get('hobby')      # [추가됨] 폼에서 취미 데이터 받아오기
        )
...
def login():
    if request.method == 'POST':
        ...
        if password_ok:
            ...
            session['hobby'] = user.hobby        # [추가됨] 로그인 시 세션에 취미 저장
