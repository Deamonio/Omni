...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://s2501003:sjwwa951@localhost/student_db'
...
class User(db.Model):
    ...
    post = db.Column(db.String(5)) #신규
    hobby = db.Column(db.String(100)) #기말고사
...
def signup():
    if request.method == 'POST':
        ...
        new_user = User(
            ...
            address = request.form.get('address'), #신규
...
def login():
    if request.method == 'POST':
        ...
        if password_ok:
            ...
            session['address'] = user.address #신규
            session['post'] = user.post #신규
            session['hobby'] = user.hobby #기말고사
...
def dashboard():
    ...
    if session['is_admin']:
        ...
        return render_template('student.html', grade=grade, user=user)
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
    # PORT가 지정되면 우선 사용하고, 없으면 5000부터 빈 포트를 자동 선택
